...
async function lookupDnitKm(lat, lng, label) {
  ...
  try {
    ...
    if (data && typeof data === 'object') {
      const km  = data.km ?? data.Km ?? data.KM;
      const br  = data.rodovia ?? data.br ?? data.Rodovia ?? data.BR;
      const uf  = data.uf ?? data.UF;
      const bits = [];
      if (br) bits.push(`BR-${br}`);
      if (km != null) bits.push(`km ${km}`);
      if (uf) bits.push(uf);
      const summary = bits.length ? bits.join(' · ') : JSON.stringify(data).slice(0, 140);
      showToast(`📍 DNIT — <span class="accent">${label}</span>: ${summary}`);
    } else if (data) {
      showToast(`📍 DNIT — <span class="accent">${label}</span>: ${String(data).slice(0, 140)}`);
    } else {
      showToast(`⚠️ DNIT — resposta vazia para <span class="accent">${label}</span>`);
    }
    return data;
  } catch (err) {
    console.error('DNIT localizarkm lookup failed:', err);
    showToast(`⚠️ Falha na consulta DNIT (${label})`);
    return null;
  }
}
...
function runDnitLookupForLayer(parsedLayer) {
  ...
  Object.values(parsedLayer._layers || {}).forEach(l => {
    ...
    sublayers.forEach(sl => {
      ...
      if (latlng && name.includes('LD_INICIO')) { // catches both LD_INICIO and LD_INICIO_OAE
        matches.push({ name: props.name || name, latlng });
      }
    });
  });
  matches.forEach(m => lookupDnitKm(m.latlng.lat, m.latlng.lng, m.name));
}
